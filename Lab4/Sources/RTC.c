...
{
//  TODO: Figure out how to do the lock register stuff --  allow for reset on battery disconnect.
//  if ( RTC_SR & RTC_SR_TIF_MASK )
//    {
//      // Clear the time-invalid flag
//      RTC_CR &= ~RTC_SR_TIF_MASK;
...
//      RTC_CR = ~RTC_CR_SWR_MASK;
    ...
    // Wait for oscillators to stabilise
    ...
    // If control, status, or lock register is unlocked
    if ((RTC_LR & RTC_LR_CRL_MASK == RTC_LR_CRL_MASK) ||
        (RTC_LR & RTC_LR_SRL_MASK == RTC_LR_SRL_MASK) ||
        (RTC_LR & RTC_LR_LRL_MASK == RTC_LR_LRL_MASK))
    {
